...
def plot_gradient(grad_xyz: np.ndarray, floor_height: int, threshold: float = 0.1, alpha: float = 0.5):
...
def plot_wavefront_position(pos: np.ndarray, dir: np.ndarray, ior_field: np.ndarray, title: str, 
...
def plot_irradiance_grid(radiometric_grid: np.ndarray, floor_height: int, threshold=3.0):
...
def plot_irradiance_slices(radiometric_grid: np.ndarray, threshold=3.0, num_slices=8, z_start=None, z_end=None):    
    if z_start is None:
        z_start = 0
    if z_end is None:
